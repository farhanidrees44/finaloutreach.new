  ...
  return (
    <section id="about" className="border-t border-ink-08 bg-background">
      <div className="mx-auto max-w-7xl px-6 py-20 md:px-10 md:py-24 lg:px-12">
        ...
        {/* Team grid — named operators with verifiable Upwork/LinkedIn */}
        <div className="mt-16 border-t border-ink-08 pt-12 md:mt-20">
          <div className="flex items-end justify-between gap-4">
            <div>
              <h3 className="type-h3 text-ink">The operators</h3>
              <p className="mt-2 max-w-md text-[14px] text-ink-60">
                Real people, real profiles — hover a card for specialty and
                links.
              </p>
            </div>
          </div>
          <ul className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {TEAM.map((m, i) => (
              <motion.li
                ...
                className="group"
              >
                <div className="relative aspect-[4/5] w-full overflow-hidden rounded-2xl bg-cream shadow-premium-sm transition-shadow duration-300 group-hover:shadow-premium-md">
                  <Image
                    src={m.img}
                    alt={m.name}
                    fill
                  ...
                  />
                  <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-ink/90 via-ink/40 to-transparent p-4 opacity-0 transition-opacity duration-400 group-hover:opacity-100">
                    <p className="type-label text-background/70">Specialty</p>
                    <p className="mt-1 text-[13px] font-semibold leading-snug text-background">
                      {m.specialty}
                    </p>
                    <div className="mt-3 flex translate-y-2 flex-wrap gap-2 opacity-0 transition-all duration-400 group-hover:translate-y-0 group-hover:opacity-100">
                      {m.upwork && (
                        <a
                          href={m.upwork}
                          target="_blank"
                          ...
                          className="rounded-full bg-[#14a800] px-2.5 py-1 text-[11px] font-semibold text-white"
                      ...
                      {m.linkedin && (
                        <a
                          ...
                          className="inline-flex items-center gap-1 rounded-full bg-[#0077b5] px-2.5 py-1 text-[11px] font-semibold text-white"
                          onClick={(e) => e.stopPropagation()}
                        >
                ...
                <div className="mt-4 flex items-baseline justify-between gap-2">
                  ...
                  <p className="font-mono text-[11px] text-ink-40">
                    <span className="proof">{m.years}y</span>
                  </p>
                </div>
                <p className="text-[13px] text-ink-60">{m.role}</p>
              </motion.li>
            ))}
          </ul>
        </div>
